...
async function moveSelectedImages() {
  ...
  // selectedDir is now already a full path from the radio button data-path attribute
  // But handle legacy case where it might still be just a folder name
  if (destFolder && !require('path').isAbsolute(destFolder) && dirFolder) {
        ...
        <div class="modal-body">
          <div class="mb-3">
            ...
            <div class="btn-group-vertical w-100" role="group">
              <input type="radio" class="btn-check" name="sortBy" id="sortByName" value="name" autocomplete="off">
              <label class="btn btn-outline-light" for="sortByName">Name</label>
              
              <input type="radio" class="btn-check" name="sortBy" id="sortByDate" value="date" autocomplete="off">
              <label class="btn btn-outline-light" for="sortByDate">Date</label>
              
              <input type="radio" class="btn-check" name="sortBy" id="sortBySize" value="size" autocomplete="off">
              <label class="btn btn-outline-light" for="sortBySize">Size</label>
            </div>
          </div>
          <div class="mb-3">
            <label class="form-label">Sort Order:</label>
            <div class="btn-group-vertical w-100" role="group">
              <input type="radio" class="btn-check" name="sortOrder" id="sortOrderAsc" value="asc" autocomplete="off">
              <label class="btn btn-outline-light" for="sortOrderAsc">Ascending</label>
            
              <input type="radio" class="btn-check" name="sortOrder" id="sortOrderDesc" value="desc" autocomplete="off">
              <label class="btn btn-outline-light" for="sortOrderDesc">Descending</label>
            </div>
          </div>
        </div>
        <div class="modal-footer">
...
// Wire Sort-by button to open sort modal
(function bindSortButton(){
  ...
  function updateSortDisplay() {
    if (sortTypeSpan) {
      const byText = sortBy === 'name' ? 'Name' : sortBy === 'date' ? 'Date' : 'Size';
      const orderText = sortOrder === 'asc' ? '?' : '?';
      sortTypeSpan.textContent = ` (${byText} ${orderText})`;
    }
  }
  ...
  if (sortBtn && !sortBtn._sortBound){
    sortBtn.addEventListener('click', (e)=>{
      ...
      document.getElementById(`sortBy${sortBy.charAt(0).toUpperCase() + sortBy.slice(1)}`).checked = true;
      document.getElementById(`sortOrder${sortOrder.charAt(0).toUpperCase() + sortOrder.slice(1)}`).checked = true;
      sortModal.show();
    });
    sortBtn._sortBound = true;
  }
  
  // Wire Refresh list button
  ...
  if (applyBtn && !applyBtn._sortBound) {
    applyBtn.addEventListener('click', async () => {
      const newSortBy = document.querySelector('input[name="sortBy"]:checked')?.value || 'name';
      const newSortOrder = document.querySelector('input[name="sortOrder"]:checked')?.value || 'asc';
